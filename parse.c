...
static node_t *build_leaf(void) {
    node_t* nodeToAdd = calloc(1, sizeof(node_t));
        nodeToAdd->node_type = NT_LEAF;
        nodeToAdd->tok = this_token->ttype;
        if(this_token->ttype == TOK_NUM) {
            nodeToAdd->type = INT_TYPE;
            nodeToAdd->val.ival = atoi(this_token->repr);
        }
        else if(this_token->ttype == TOK_TRUE) {
            nodeToAdd->type = BOOL_TYPE;
            nodeToAdd->val.bval = true;
        }
        else if(this_token->ttype == TOK_FALSE) {
            nodeToAdd->type = BOOL_TYPE;
            nodeToAdd->val.bval = false;
        }
        else if(this_token->ttype == TOK_STR) {
            nodeToAdd->type = STRING_TYPE;
            nodeToAdd->val.sval = (char *)malloc(strlen(this_token->repr)+1);
            strcpy(nodeToAdd->val.sval, this_token->repr);
        }
        else if(this_token->ttype == TOK_ID) {
            nodeToAdd->type = ID_TYPE;
            nodeToAdd->tok = TOK_ID;
            nodeToAdd->val.sval = (char *)malloc(strlen(this_token->repr)+1);
            strcpy(nodeToAdd->val.sval, this_token->repr);
        }
        else if(this_token->ttype == TOK_FMT_SPEC ) {
            nodeToAdd->type = FMT_TYPE;
            nodeToAdd->tok = this_token->ttype;
            nodeToAdd->val.fval = this_token->repr[0];
        }
